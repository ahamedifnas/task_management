  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-slate-900 dark:text-white text-2xl font-bold">Monthly Reports</h1>
          <p className="text-slate-600 dark:text-slate-400 text-sm mt-0.5">Generate and export overtime payroll summaries</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={generating}
            className="flex items-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-60 text-white text-sm font-medium rounded-lg transition-colors duration-300"
          >
      ...
      {!otPolicy && (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/30 rounded-xl px-5 py-4 flex items-start gap-3">
          <HiExclamationTriangle className="w-5 h-5 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
          <div>
            <p className="text-amber-600 dark:text-amber-400 text-sm font-medium">No OT policy configured</p>
            <p className="text-amber-700 dark:text-amber-300/70 text-xs mt-1">Configure an OT policy before generating reports.</p>
          </div>
        </div>
      )}
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700/50 p-4 transition-colors duration-300">
              <p className="text-slate-600 dark:text-slate-400 text-xs mb-1">Employees</p>
              <p className="text-slate-900 dark:text-white text-xl font-bold">{enriched.length}</p>
            </div>
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700/50 p-4 transition-colors duration-300">
              <p className="text-slate-600 dark:text-slate-400 text-xs mb-1">Total OT Hours</p>
              <p className="text-amber-600 dark:text-amber-400 text-xl font-bold">
                {minutesToHHMM(enriched.reduce((s, r) => s + (r.overtimeMin || 0), 0))}
              </p>
            </div>
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700/50 p-4 transition-colors duration-300">
              <p className="text-slate-600 dark:text-slate-400 text-xs mb-1">Total OT Amount</p>
              <p className="text-emerald-600 dark:text-emerald-400 text-xl font-bold">LKR {totalOTAmount.toFixed(0)}</p>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-emerald-700 hover:bg-emerald-600 text-white text-sm font-medium rounded-lg transition-colors duration-300"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-red-700 hover:bg-red-600 text-white text-sm font-medium rounded-lg transition-colors duration-300"
            >
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-700/50">
                  ...
                  <th className="text-left px-5 py-3 text-slate-600 dark:text-slate-400 text-xs font-medium uppercase tracking-wider">Basic Salary</th>
                  ...
                  <th className="text-left px-5 py-3 text-slate-600 dark:text-slate-400 text-xs font-medium uppercase tracking-wider">OT Amount</th>
                  <th className="text-left px-5 py-3 text-slate-600 dark:text-slate-400 text-xs font-medium uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-200 dark:divide-slate-700/50">
                {enriched.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-100 dark:hover:bg-slate-700/20 transition-colors duration-300">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-slate-800 dark:text-slate-200 text-sm font-medium">{row.employeeName}</span>
                      </div>
                    </td>
                    <td className="px-5 py-4 text-slate-600 dark:text-slate-400 text-sm">{row.department}</td>
                    <td className="px-5 py-4 text-slate-700 dark:text-slate-300 text-sm">
                      LKR {Number(row.basicSalary || 0).toLocaleString()}
                    </td>
                    <td className="px-5 py-4 text-sky-600 dark:text-sky-400 text-sm font-medium">{minutesToHHMM(row.totalWorkMin)}</td>
                    <td className="px-5 py-4">
                      <span className={`text-sm font-medium ${row.overtimeMin > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-slate-500'}`}>
              ...
              <tfoot>
                <tr className="border-t border-slate-300 dark:border-slate-600 bg-slate-100/70 dark:bg-slate-900/50">
                  <td colSpan={5} className="px-5 py-3 text-slate-700 dark:text-slate-300 text-sm font-semibold text-right">Total OT Payable:</td>
                  <td className="px-5 py-3 text-emerald-600 dark:text-emerald-400 text-sm font-bold">LKR {totalOTAmount.toFixed(2)}</td>
