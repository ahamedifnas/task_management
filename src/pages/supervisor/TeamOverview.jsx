    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">Monthly summary of your team's work</p>
        </div>
        <input
          type="month"
          value={selectedMonth}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700/50">
                  ...
                  <th className="text-left px-5 py-3 text-slate-400 text-xs font-medium uppercase tracking-wider">Days Logged</th>
                  ...
                  <th className="text-left px-5 py-3 text-slate-400 text-xs font-medium uppercase tracking-wider">Approved</th>
                  <th className="text-left px-5 py-3 text-slate-400 text-xs font-medium uppercase tracking-wider">Pending</th>
                </tr>
              </thead>
                ...
                {employees.map((emp) => {
                  ...
                  return (
                    <tr key={emp.id} className="hover:bg-slate-700/20 transition-colors">
                      ...
                      <td className="px-5 py-4 text-slate-300 text-sm">{days.length}</td>
                      <td className="px-5 py-4 text-sky-400 text-sm font-medium">{minutesToHHMM(totalWork)}</td>
                      <td className="px-5 py-4">
                        <span className={`text-sm font-medium ${totalOT > 0 ? 'text-amber-400' : 'text-slate-500'}`}>
                      ...
                      <td className="px-5 py-4">
                        <span className={`text-sm font-medium ${pending > 0 ? 'text-amber-400' : 'text-slate-500'}`}>
                          {pending}
                        </span>
                      </td>
                    </tr>
