  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-slate-900 dark:text-white text-2xl font-bold">Daily Timesheet</h1>
          <p className="text-slate-600 dark:text-slate-400 text-sm mt-0.5">Log your tasks and track working hours</p>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700/50 p-4 transition-colors duration-300">
              <p className="text-slate-600 dark:text-slate-400 text-xs mb-1">Total Work</p>
              <p className="text-sky-600 dark:text-sky-400 text-xl font-bold">{minutesToHHMM(totalMin)}</p>
            </div>
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700/50 p-4 transition-colors duration-300">
              <p className="text-slate-600 dark:text-slate-400 text-xs mb-1">Standard ({otPolicy?.stdHoursPerDay || 8}h)</p>
              <p className="text-slate-700 dark:text-slate-300 text-xl font-bold">{minutesToHHMM(stdMin)}</p>
            </div>
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700/50 p-4 transition-colors duration-300">
              <p className="text-slate-600 dark:text-slate-400 text-xs mb-1">Overtime</p>
              <p className={`text-xl font-bold ${otMin > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-slate-500'}`}>
                {minutesToHHMM(otMin)}
              </p>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700/50 transition-colors duration-300">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-slate-700/50">
              <h2 className="text-slate-900 dark:text-white font-semibold">Tasks ({tasks.length})</h2>
              {!isLocked && (
                <button
                  onClick={openNewTask}
                  className="flex items-center gap-2 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm rounded-lg transition-colors duration-300"
                >
            ...
            {tasks.length === 0 ? (
              <div className="py-12 text-center">
                <HiClock className="w-10 h-10 text-slate-500 dark:text-slate-600 mx-auto mb-3" />
                <p className="text-slate-600 dark:text-slate-400 font-medium">No tasks yet</p>
                    ...
                    <div key={task.id} className="px-5 py-4">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          ...
                          {proj && (
                            <p className="text-indigo-600 dark:text-indigo-400 text-xs font-medium mb-0.5">{proj.projectName}</p>
                          )}
                          <p className="text-slate-800 dark:text-slate-200 text-sm">{task.description}</p>
                          {task.workReport && (
                            <p className="text-slate-600 dark:text-slate-400 text-xs mt-1 line-clamp-2">{task.workReport}</p>
                          )}
                        </div>
                        {!isLocked && (
                          <div className="flex gap-1 shrink-0">
                            <button
                              ...
                              className="p-1.5 text-slate-600 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-slate-200 dark:hover:bg-slate-700 rounded transition-colors duration-300"
                            >
                              <HiPencilSquare className="w-4 h-4" />
                            </button>
                            <button
                              onClick={() => deleteTask(task.id)}
                              className="p-1.5 text-slate-600 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-slate-200 dark:hover:bg-slate-700 rounded transition-colors duration-300"
                            >
          ...
          {workday?.status === 'REJECTED' && (
            <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700/50 rounded-xl px-5 py-4">
              <div className="flex items-center gap-2 mb-1">
                <HiXCircle className="w-4 h-4 text-red-600 dark:text-red-400" />
                <p className="text-red-600 dark:text-red-400 font-medium text-sm">Rejected</p>
              </div>
              {workday.rejectionComment && (
                <p className="text-red-700 dark:text-red-300/70 text-sm mt-1">{workday.rejectionComment}</p>
              )}
              <p className="text-slate-600 dark:text-slate-400 text-xs mt-2">Update your tasks and resubmit.</p>
            </div>
          )}
          ...
          {!isLocked && tasks.length > 0 && workday?.status !== 'REJECTED' && (
            <button
              ...
              disabled={submitting}
              className="w-full flex items-center justify-center gap-2 py-3 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-60 text-white font-semibold rounded-xl transition-colors duration-300 shadow-lg shadow-emerald-500/20"
            >
          ...
          {workday?.status === 'SUBMITTED' && (
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/50 rounded-xl px-5 py-3 text-center flex items-center justify-center gap-2">
              <HiClock className="w-4 h-4 text-amber-600 dark:text-amber-400" />
              <p className="text-amber-600 dark:text-amber-400 font-medium text-sm">Awaiting Supervisor Approval</p>
            </div>
          )}

          {workday?.status === 'APPROVED' && (
            <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700/50 rounded-xl px-5 py-3 text-center flex items-center justify-center gap-2">
              <HiCheckCircle className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
              <p className="text-emerald-600 dark:text-emerald-400 font-medium text-sm">Approved</p>
            </div>
          )}
        </>
      )}
      ...
      >
        <form onSubmit={handleSubmit(onTaskSubmit)} className="space-y-4">
          <div>
            <label className="block text-slate-700 dark:text-slate-300 text-sm font-medium mb-1.5">Project</label>
            <select
            ...
            {errors.projectId && <p className="text-red-600 dark:text-red-400 text-xs mt-1">{errors.projectId.message}</p>}
          </div>

          <div>
            <label className="block text-slate-700 dark:text-slate-300 text-sm font-medium mb-1.5">Description</label>
            <input
              type="text"
            ...
            />
            {errors.description && <p className="text-red-600 dark:text-red-400 text-xs mt-1">{errors.description.message}</p>}
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-slate-700 dark:text-slate-300 text-sm font-medium mb-1.5">Start Time</label>
              <input
                type="time"
                className="w-full px-3 py-2.5 bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors duration-300"
                {...register('startTime', { required: 'Required' })}
              />
              {errors.startTime && <p className="text-red-600 dark:text-red-400 text-xs mt-1">{errors.startTime.message}</p>}
            </div>
            <div>
              <label className="block text-slate-700 dark:text-slate-300 text-sm font-medium mb-1.5">End Time</label>
              <input
                type="time"
                className="w-full px-3 py-2.5 bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors duration-300"
                {...register('endTime', { required: 'Required' })}
              />
              {errors.endTime && <p className="text-red-600 dark:text-red-400 text-xs mt-1">{errors.endTime.message}</p>}
            </div>
          </div>

          {duration > 0 && (
            <div className="bg-slate-100/70 dark:bg-slate-900/50 rounded-lg px-3 py-2 text-sm text-slate-600 dark:text-slate-400">
              Duration: <span className="text-amber-600 dark:text-amber-400 font-medium">{minutesToHHMM(duration)}</span>
            </div>
          )}

          <div>
            <label className="block text-slate-700 dark:text-slate-300 text-sm font-medium mb-1.5">Work Report</label>
            <textarea
              rows={3}
              placeholder="Describe what you accomplished..."
              className="w-full px-3 py-2.5 bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none transition-colors duration-300"
              {...register('workReport')}
            />
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button
              ...
              className="flex-1 py-2.5 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:border-slate-400 dark:hover:border-slate-500 rounded-lg text-sm transition-colors duration-300"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={submitting}
              className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-60 text-white font-medium rounded-lg text-sm transition-colors duration-300"
            >
